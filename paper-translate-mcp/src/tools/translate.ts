...
async function translateFigureCaptions(
  ...
  for (const fig of figures) {
    const translatedCaption = await translateCaption(fig.caption, 'figure', sampler, targetLanguage);
    translated.push({
      ...fig,
      translatedCaption,
    });
...
async function translateTableCaptions(
  ...
  for (const table of tables) {
    const translatedCaption = await translateCaption(table.caption, 'table', sampler, targetLanguage);
    translated.push({
      ...table,
      translatedCaption,
    });
